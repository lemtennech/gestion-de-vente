...
/**jdbc:mysql://localhost:3306/gestion_de_produit?zeroDateTimeBehavior=convertToNull
 *
 * @author AmineGasa
 */
public class ConnectDB {
    ...
    private final String password = "010203";
    String ipLocal = "localhost";

    private String url = "jdbc:mysql://localhost:3306/gestion_de_produit?zeroDateTimeBehavior=convertToNull";
    ...
    public Connection ConnexiontoDataBase() {
    ...
    public void insertFornisseur(Fornisseur forn) {
        ...
        try {
            st = cnx.createStatement();
            String sql1 = "INSERT INTO `fornisseur`( `nom_prenom`, `N_telephone`, `Adresse`,`credit`) "
                    + "VALUES ('" + forn.nom_prenom + "'," + "'" + forn.telephone + "','" + forn.adresse + "','" + forn.credit + "') ";
    ...
    public void insertClient(Client cl) {
        try {
            st = cnx.createStatement();
            String sql1 = "INSERT INTO `client`( `nom_prenom`, `N_telephone`, `Adresse`,`credit`) "
                    + "VALUES ('" + cl.nom_prenom + "'," + "'" + cl.telephone + "','" + cl.adresse + "','" + cl.credit + "') ";
            st.executeUpdate(sql1);
        } catch (SQLException ex) {
            Logger.getLogger(ConnectDB.class.getName()).log(Level.SEVERE, null, ex);
            JOptionPane.showMessageDialog(null, "ereur operation");
    ...
    public void AllFornisseur(JTable T) {
        ...
        try {
            ArrayList<Fornisseur> list = new ArrayList<Fornisseur>();
            st = cnx.createStatement();
            ...
            while (rst.next()) {
                ...
                Fornisseur fo = new Fornisseur(rst.getInt("N_forn"), rst.getString("nom_prenom"), rst.getString("N_telephone"), rst.getString("Adresse"),
                        rst.getFloat("credit"));
                list.add(fo);
            }
            ...
            for (int k = 0; k < list.size(); k++) {
                row[0] = list.get(k).getN_for();
                row[1] = list.get(k).getNom_prenom();
                row[2] = list.get(k).getTelephone();
                row[3] = list.get(k).getAdresse();
                row[4] = list.get(k).getCredit();
       ...
       try { 
           ...
           int l =0;
              while (rst.next()) {
                ...
                row[0] = rst.getInt("N_client");
                row[1] = rst.getString("nom_prenom");
                row[2] = rst.getString("N_telephone");
                row[3]= rst.getString("Adresse");
                row[4]= rst.getFloat("credit");
                mo.addRow(row);
                l++;
            }
    ...
    public void UpdatForn(int id, Fornisseur fr) {
        ...
        try { st = cnx.createStatement();
            String sql = "Update fornisseur set nom_prenom='" + fr.nom_prenom + "',N_telephone='" + fr.telephone + "',"
                    + "Adresse='" + fr.adresse + "'" + ",credit='" + fr.credit + "'"
                    + " where N_forn=" + id;
            st.executeUpdate(sql);
        } catch (SQLException ex) {
            Logger.getLogger(ConnectDB.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public void UpdatClient(int id, Client cl) {
        ...
        try { st = cnx.createStatement();
            String sql = "Update client set  nom_prenom='" + cl.nom_prenom + "',N_telephone='" + cl.telephone + "',"
                    + "Adresse='" + cl.adresse + "',"+ "credit='" + cl.credit +"' " 
                    + "where N_client=" + id;
    ...
    public void FindFornisseur(JTable T, String recherche) {
        try {
            ArrayList<Fornisseur> list = new ArrayList<Fornisseur>();
            st = cnx.createStatement();
            String sql = "SELECT * FROM fornisseur WHERE nom_prenom like '" + recherche + "%'  ";
            rst = (ResultSet) st.executeQuery(sql);
            int i = 0, j;
           while (rst.next()) {
    ...
    public void FindDesignation(JComboBox combo) {
        ...
        try {
            st = cnx.createStatement();
            String sql = "select designation  from stock";
            rst = (ResultSet) st.executeQuery(sql);
            ArrayList<String> ar = new ArrayList<>();
            ArrayList<String> rpl = new ArrayList<>();
        ...
        try {
            ...
            while (rst.next()) {
                ...
                Achat ach = new Achat(rst.getInt("N_achat"), rst.getString("nom_prenom_forn"), rst.getString("designation"), rst.getInt("quantite"),
                        ...
                        rst.getDate("date_achat") + "",rst.getInt("fact_id"));
                list.add(ach);
            }
    ...
    public void inserAchat(Achat ach) {
        try {
            ...
            String sql1 = "INSERT INTO `achat`( `nom_prenom_forn`, `designation`, `quantite`"
                    + ", `prix_total`, `prix_unitaire`, `date_achat`, `fact_id`) "
                    + "VALUES ('" + ach.nom_prenom_forn + "'," + "'" + ach.designation + "','" + ach.quantité + "'"
                    + ",'" + ach.prix_unitaire*ach.quantité + "','" + ach.prix_unitaire + "','" + ach.date_achat + "','" + ach.fact_id + "') ";
            st.executeUpdate(sql1);
        } catch (SQLException ex) {
            Logger.getLogger(ConnectDB.class.getName()).log(Level.SEVERE, null, ex);
            JOptionPane.showMessageDialog(null, "ereur operation");
    ...
    public void UpdatAchat(int id, Achat ach) {
        ...
        try {
            String sql = "Update achat set nom_prenom_forn='" + ach.nom_prenom_forn + "',designation='" + ach.designation + "',"
                    ...
                    + "prix_unitaire='" + ach.prix_total / ach.quantité + "'"+ "fact_id='" + ach.fact_id
                    + "where N_achat=" + id;
            st.executeUpdate(sql);
        } catch (SQLException ex) {
            Logger.getLogger(ConnectDB.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public void DeleteAchat(int id) {
        ...
        public void Alerte(JTable T) {

        try {
            ArrayList<Stock> list = new ArrayList<Stock>();
            st = cnx.createStatement();
            String sql = "select * from stock where qnt <= alerte";
            rst = (ResultSet) st.executeQuery(sql);
            DES.clear();
            while (rst.next()) {
                ...
                Stock ST = new Stock(rst.getInt("N_prod"), rst.getString("designation"),
                        ...
                        rst.getInt("unite_d") , rst.getInt("qnt"),
                        rst.getInt("alerte"), rst.getFloat("prix_achat"),
                        rst.getFloat("prix_vent")
                );
                list.add(ST);
            }
            ...
            for (int k = 0; k < list.size(); k++) {
                ...
                row[1] = list.get(k).getCategorie();
                DES.add(list.get(k).getDesignation());
                row[2] = list.get(k).getDesignation();
                row[3] = list.get(k).getUnite();
                row[4] = list.get(k).getUnite_d();
                row[5] = list.get(k).getQnt();
                row[6] = list.get(k).getAlerte();
                row[7] = list.get(k).getPrix_achat();
                row[8] =  list.get(k).getPrix_vent();
    ...
    public void inserFacture(String nom_prenom_forn, String date) {
        try {
    ...
    public void inserFacture_v(String nom_prenom_forn, String date) {
        try {
           
            st = cnx.createStatement();
            String sql1 = "INSERT INTO `fact_v`( `client`, `somme`,`date`) "
                    + "VALUES ('" + nom_prenom_forn + "'," + 0  + ",'" + showDate() + "') ";
            st.executeUpdate(sql1);
        } catch (SQLException ex) {
            Logger.getLogger(ConnectDB.class.getName()).log(Level.SEVERE, null, ex);
   ...
   public void UpdatFacture(int id, int montant) {

        try {
            String sql = "Update facture set montant_facture='" + montant + "'"
   ...
   public void UpdatFacture_v(int id, float montant, float credit, float verment) {
    ...
    public String FindFact() {
      String r = null;
        try {
           
            st = cnx.createStatement();
            
            rst = (ResultSet) st.executeQuery("SELECT N_fact FROM facture ORDER BY N_fact DESC LIMIT 1;");

            while (rst.next()) {
                r = Integer.toString(rst.getInt("N_fact"));
            }
     ...
     public String FindFact_v() {
      String r = null;
        try {
           
            st = cnx.createStatement();
            
            rst = (ResultSet) st.executeQuery("SELECT n_fact FROM fact_v ORDER BY N_fact DESC LIMIT 1;");
      ...
      public int Find_Last_v() {
      int r = 0;
        try {
           
            st = cnx.createStatement();
            
            rst = (ResultSet) st.executeQuery("SELECT N_vente FROM vente ORDER BY N_vente DESC LIMIT 1;");

            while (rst.next()) {
                r = rst.getInt("N_vente");
            }
    ...
    public void FindDesignationAchat(JComboBox combo) {
        try {
            st = cnx.createStatement();
...
public void FindPrix(String des, JTextField jt) {
     
        try {
            
            String sql = "select prix_vent from stock where designation = '"+des +"'";
            ...
            if (rst.next()) {
                ...
                float f = rst.getFloat("prix_vent");
                jt.setText(/*df.format(*/f/*)*/ + "");
            }
...
public int Findant(String des) {
     int qnt = 0 ;
        try {
            ...
            String sql = "select qnt from stock where designation = '"+des +"'";
    ...
    public void insertStock(Stock ST) {
        ...
        try {
            ...
            String sql;
                ...
                sql = "INSERT INTO `gestion_de_produit`.`stock` (`designation`, `categorie`, `unite`, `prix_vent`, `qnt`, `alerte`, `unite_d`, `prix_achat`) "
                        + "VALUES ('" + ST.getDesignation() + "','" + ST.getCategorie()
                        + "','" + ST.getUnite() + "','" + ST.getPrix_vent() 
                        + "','" + ST.getQnt() + "','" + ST.getAlerte()+ "','" + ST.getUnite_d() + "','" + ST.getPrix_achat()+"') ";
  ...
  /*  public void UpdatStock(String sql, int n) {
    ...
    public String Get_credit(String nom) throws SQLException{
        String credit = null;
        
        st = cnx.createStatement();
        String sql = "select credit from client where nom_prenom = '"+nom+"'";
        rst = st.executeQuery(sql);
         while (rst.next()) {
